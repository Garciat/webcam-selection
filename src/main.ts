...
class MotionDetector {
    ...
    constructor(private video: HTMLVideoElement, private threshold: number, private debug?: boolean) {
        this.video = video;
...
class WebcamSelectionAdapter {
    ...
    onSelected = (id: number, source: HTMLElement) => {};
    
    constructor(webcam: WebcamDisplay, private debug?: boolean) {
        this.root = WebcamSelectionAdapter.createRoot();
        
        this.video = webcam.getVideo();
        this.md = new MotionDetector(this.video, 50, true);
    ...
    addButton(id: number, x: number, y: number, button: HTMLElement, threshold: number) {
        ...
        this.bcd.register({
            ...
            callback: () => this.onSelected(id, button)
        });
    }
...
class SelectionGame {
    private adapter: WebcamSelectionAdapter;
    
    onLost = () => {};
    
    constructor(webcam: WebcamDisplay, private debug?: boolean) {
        this.adapter = new WebcamSelectionAdapter(webcam, debug);
    }
    ...
    loadTemplate(template: HTMLTemplateElement) {
        ...
        for (var button of buttons) {
            ...
            this.adapter.addButton(id, x, y, button, 0.05);
        }
    }
    ...
    beginRound(sequence: number[]) {
        ...
        this.adapter.onSelected = (selectedId) => {
            if (waiting) return;
            
            var expected = sequence[0];
            
            console.log(selectedId, expected);
...
window.addEventListener('load', function () {
    ...
    webcam.onReady = () => {
        var game = new SelectionGame(webcam, true);
        game.loadTemplate(template);
        
        game.onLost = () => console.log('lost');
        
        setTimeout(function () {
            game.beginRound([1, 2, 3]);
            console.log('begin!');
        }, 5000);
    };
});
